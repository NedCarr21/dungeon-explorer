...
module Input
  ...
  def self.buttonToKey(button)
    ...
    when Input::A;         return [0x5A,0x10]           # Z, Shift
    when Input::B;         return [0x58,0x1B]           # X, ESC
    when Input::C;         return [0x43,0x0D]           # C, ENTER
    when Input::Y;         return [0x41,0x51]           # A, Q
    when Input::Z;         return [0x53,0x57]           # S, W
    when Input::Z;         return [0x44,0x46]           # D, F
...
if mkxp?
  module Mouse
    def self.setCapture(catch_anywhere = false)
